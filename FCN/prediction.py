...
model = torch.load('220301/checkpoints_colorjitter/fcn_model_4.pt')  # 加载模型
...
if __name__ =='__main__':
    img_file = 'G:/ECU/Original/002'
    # races_file = 'G:/RCW_test/images'
    # races = os.listdir(races_file)
    img_names = os.listdir(img_file) 
    # for race in races:
    #     print(race)
    #     img_file = os.path.join(races_file,race)
    #     img_names = os.listdir(img_file)
        
    save_file = '220301/colorjitter_ECU/sigmoid'
    ...
    for img_name in tqdm(img_names):
        ...
        # print(output.shape, output,'after sigmoid')
        
        output_np = output.cpu().detach().numpy().copy() 
        # print(output_np.shape,'after cpu')
        output_np = np.squeeze(output_np)*255
        # output_np = np.argmin(output_np,axis=0)
        ...
        # print(output_np)
        # plt.subplot(1,2,1)
        # plt.imshow(output_npA,'gray')
        
        # plt.subplot(1,2,2)
        # plt.imshow(output_sigmoid,'gray')
        # plt.show()
